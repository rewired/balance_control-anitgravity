...
import { isAdjacent } from '@balance-control/game';
...
export function isMoveAdjacent(G: GameState, sourceId: string, targetId: string): boolean {
    ...
    // (a) Adjacent(A,B) = true
    if (isAdjacent(G, sourceId, targetId)) {
        return true;
    }
    ...
    if (startCommitteeId && isAdjacent(G, sourceId, startCommitteeId)) {
        if (isAdjacent(G, startCommitteeId, targetId)) {
            return true;
        }
    }
