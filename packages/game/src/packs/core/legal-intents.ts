...
import type { LegalIntent } from '../pack-api';
import {
    ...
    stringToCoord,
    EffectResolver,
    evaluateTileSelector,
    EnginePackRegistry,
    getPlayerMetaMarker,
    getLegalGrassrootsOutputs,
    selectDeterministicCostResourceIds,
} from '../pack-api';
import { isMoveAdjacent } from './adjacency';

type CostSlot = string[] | 'ANY';

/**
 * CORE legal-intent enumeration: draw-and-place tile placement and
 * political-action moves (place/move/formalize influence, convert
 * resources, take measure), plus pending-choice resolution options.
 * @rule CORE-01-04-05A
 * @rule CORE-01-04-09
 * @rule CORE-01-04-08
 * @deterministic
 * @pure
 */
export function coreEnumerateIntents(G: GameState, ctx: any, playerID: string): LegalIntent[] {
    const pendingChoice = G.engine?.pendingChoice;
    if (pendingChoice) {
        return buildResolveChoiceIntents(G, ctx, pendingChoice);
    }
