...
exports.Expansion01 = {
    ...
    onSetup: (G, ctx) => {
        if (G.secret)
            G.secret.playedMeasureThisRound = {};
        ...
        const playerIds = Object.keys(G.zones)
            .filter(z => z.startsWith(rules_1.CoreZoneNames.PersonalSupply))
            .map(z => z.split(':')[1]);
        ...
        const addEcoResort = (weight, count) => {
            for (let i = 0; i < count; i++) {
                const id = `tile_eco_w${weight}_${i}_${Math.random().toString(36).substr(2, 5)}`;
                G.tiles[id] = { id, type: rules_1.TileType.Resort, resort: 'ECO', weight, name: `ECO W${weight}` };
        ...
        G.tiles[laborMarketId] = {
            id: laborMarketId,
            type: rules_1.TileType.Hotspot,
            name: 'Labor Market',
            isHotspot: true
        };
        G.zones[rules_1.CoreZoneNames.DrawPile].items.push(laborMarketId);
        ...
        G.tiles[ipId] = { id: ipId, type: rules_1.TileType.SystemTile, name: 'Investment Program' };
        G.zones[rules_1.CoreZoneNames.Board].items.push(ipId);
        ...
        // 5. Shuffle and Deal
        G.zones.MeasureDrawPile.items = ctx.random.Shuffle(G.zones.MeasureDrawPile.items);
        for (let i = 0; i < 3; i++) {
            const mId = G.zones.MeasureDrawPile.items.pop();
            if (mId)
                G.zones.OpenMeasures.items.push(mId);
        }
        console.log('EXP-01 Setup Complete.');
    },
    modifiers: {
        production: (tileId, G, base) => {
            let amount = base;
    ...
    effectHandlers: {
        'TAKE_MEASURE': (G, ctx, effect) => {
            ...
            if (idx >= 0) {
                ...
                if (G.zones.MeasureDrawPile.items.length > 0) {
                    ...
                    if (next)
                        G.zones.OpenMeasures.items.push(next);
                }
                else if (G.zones.MeasureRecyclePile.items.length > 0) {
                    // EXP-01-07-05: If DrawPile empty, shuffle RecyclePile
                    G.zones.MeasureDrawPile.items = ctx.random.Shuffle(G.zones.MeasureRecyclePile.items);
        ...
        'PLAY_MEASURE': (G, ctx, effect) => {
            ...
            switch (mId) {
                ...
                case 'M09': // Investment Freeze
                    if (!G.secret.playerProhibitions)
                        G.secret.playerProhibitions = {};
        ...
        'CONVERT': (G, ctx, effect, utils) => {
            ...
            if (controller === playerId) {
                const ecoResources = resourceIds.filter((rid) => G.objects[rid]?.resort === 'ECO');
...
function payResources(G, playerId, cost) {
    ...
    for (const [res, amt] of Object.entries(cost)) {
        if (!inventory[res] || inventory[res].length < amt)
            return false;
    }
    for (const [res, amt] of Object.entries(cost)) {
        for (let i = 0; i < amt; i++) {
...
function recycleMeasure(G, measureObjectId, playerId) {
