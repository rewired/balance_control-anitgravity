...
exports.Expansion02 = {
    ...
    onSetup: (G, ctx) => {
        ...
        const addSecResort = (weight, count) => {
            for (let i = 0; i < count; i++) {
                const id = `tile_sec_w${weight}_${i}_${Math.random().toString(36).substr(2, 5)}`;
                G.tiles[id] = { id, type: rules_1.TileType.Resort, resort: 'SEC', weight, name: `SEC W${weight}` };
                G.zones[rules_1.CoreZoneNames.DrawPile].items.push(id);
        ...
        G.tiles[innerOrderId] = { id: innerOrderId, type: rules_1.TileType.Hotspot, name: 'Inner Order', isHotspot: true };
        G.zones[rules_1.CoreZoneNames.DrawPile].items.push(innerOrderId);
        ...
        G.tiles[authorityId] = { id: authorityId, type: rules_1.TileType.SystemTile, name: 'Authority Apparatus' };
        G.zones[rules_1.CoreZoneNames.Board].items.push(authorityId);
        ...
        ['SecurityLevel', 'Control', 'Administration', 'Blockade'].forEach((type, tIdx) => {
            for (let i = 0; i < 10; i++) {
                ...
                G.objects[id] = { id, type: 'Regulation', regType: type };
                G.zones.RegulationSupply.items.push(id);
            }
        });
        // 7. Shuffle and deal 3 Measures
        G.zones.EXP02_MeasureDrawPile.items = ctx.random.Shuffle(G.zones.EXP02_MeasureDrawPile.items);
        for (let i = 0; i < 3; i++) {
            const mId = G.zones.EXP02_MeasureDrawPile.items.pop();
            if (mId)
                G.zones.EXP02_OpenMeasures.items.push(mId);
        }
        console.log('EXP-02 Setup Complete.');
    },
    effectHandlers: {
        'TAKE_MEASURE_EXP02': (G, ctx, effect) => {
            ...
            if (idx >= 0) {
                ...
                if (G.zones.EXP02_MeasureDrawPile.items.length > 0) {
                    ...
                    if (next)
                        G.zones.EXP02_OpenMeasures.items.push(next);
                }
                else if (G.zones.EXP02_MeasureRecyclePile.items.length > 0) {
                    G.zones.EXP02_MeasureDrawPile.items = ctx.random.Shuffle(G.zones.EXP02_MeasureRecyclePile.items);
        ...
        'PLAY_MEASURE_EXP02': (G, ctx, effect, utils) => {
            const { playerId, measureObjectId } = effect.payload;
            const obj = G.objects[measureObjectId];
            if (!obj || obj.type !== 'Measure')
                return;
            const mId = obj.measureId;
            const { cost } = MEASURE_DETAILS[mId];
            // 1. Pay Cost
            if (!payResources(G, playerId, cost))
                return;
            // 2. Resolve Effects
            switch (mId) {
                ...
                case 'M11': // Risk Address: Target player may not PlayMeasure next round
                    if (!G.secret.nextRoundProhibitions)
                ...
                case 'M13': // Surveillance: Regulations on chosen Tile may not be moved or removed
                    if (!G.secret.exp02)
                        G.secret.exp02 = {};
                    if (!G.secret.exp02.protectedTiles)
                ...
                case 'M15': // Situation Assessment: Reduce cost of next Regulation placement by 1 SEC
                    if (!G.secret.playerPerks)
                        G.secret.playerPerks = {};
                    if (!G.secret.playerPerks[playerId])
                        G.secret.playerPerks[playerId] = {};
        ...
        'HOTSPOT_RESOLUTION': (G, ctx, effect, utils) => {
...
function payResources(G, playerId, cost) {
...
function placeRegulation(G, type, targetTileId, costPaid = false) {
...
function moveRegulation(G, regulationId, newTargetTileId) {
    const obj = G.objects[regulationId];
    if (!obj || obj.type !== 'Regulation')
        return;
    // Check M13 protection
    if (G.secret?.exp02?.protectedTiles?.includes(obj.targetTileId))
        return;
    obj.targetTileId = newTargetTileId;
}
function removeRegulation(G, regulationId) {
    const obj = G.objects[regulationId];
    if (!obj || obj.type !== 'Regulation')
        return;
    // Check M13 protection
...
function removeAllRegulations(G, targetTileId) {
    const attached = G.zones.BoardAttached;
...
function recycleMeasureEXP02(G, measureObjectId, playerId) {
    const obj = G.objects[measureObjectId];
