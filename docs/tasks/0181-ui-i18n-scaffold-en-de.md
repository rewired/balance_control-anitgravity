...
* [x] Step 1: Create `src/ui/i18n/` folder and add the provider + hook.
* [x] Step 2: Implement `t(key, vars?)` with `ns:path` parsing and `{{var}}` interpolation.
* [x] Step 3: Provide a locale selection strategy that is UI-only and stable:
  * default `en`
  * if `navigator.language` begins with `de`, use `de`
  * allow explicit override via `?lang=de|en` (optional)
* [x] Step 4: Wire `<I18nProvider>` at the top level (`App.tsx` or `main.tsx`).
* [x] Step 5: Add `i18n.test.ts` with the minimal coverage above.

---

## 9) Acceptance Criteria

* [x] `useT()` (or equivalent) returns a function `t(key, vars?)` usable in components.
* [x] `t('core:ui.confirm')` returns the correct EN string by default.
* [x] Missing key in DE falls back to EN.
...
* [x] Normative anchors cited for all behavior.
* [x] No engine/rule/spec changes.
* [x] `pnpm lint` passes.
* [x] `pnpm -C packages/client-web test` passes.

## 11) Work Summary
* Introduced a minimal I18N layer in `packages/client-web`.
* Created `locales.ts` with EN and DE translations exactly as defined in ARCH-06.
* Implemented `t(key, vars?)` with namespace support, `{{var}}` interpolation, and EN fallback.
* Implemented `I18nProvider` with locale detection from URL (`?lang=`) and `navigator.language`.
* Added `useI18n` and `useT` hooks for easy access to translations.
* Integrated `I18nProvider` in `main.tsx` wrapping the `App` component.
* Added comprehensive unit tests in `packages/client-web/test/i18n.test.ts`.
