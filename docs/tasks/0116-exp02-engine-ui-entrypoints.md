# Task 0116 — EXP-02 package: Add /engine and /ui entrypoints; normalize TS config to runtime-safe module format

**Date:** 2026-02-18
**Owner:** Codex
**Branch:** `task/0116-exp02-engine-ui-entrypoints`
...
* EXP-01: N/A
* EXP-02: SPEC-EXP-02 (002-expansion02.md)
* EXP-03: N/A
...
Prepare `@balance-control/expansion-02` as a pack package:
- Add `@balance-control/expansion-02/engine` and `/ui` entrypoints.
- Remove its dependency on `CoreZoneNames` for expansion zones.
- Normalize `packages/expansion-02/tsconfig.json` to a runtime-safe module output consistent with the monorepo (avoid emitting ESM into a CJS package).
...
* `packages/expansion-02/tsconfig.json` (currently sets `module: ESNext`)
...
* New files:
  ...
  - `packages/expansion-02/src/ui/index.ts`
* Updated `packages/expansion-02/src/index.ts` to re-export from `./engine`.
* Updated `packages/expansion-02/tsconfig.json` to align with the workspace runtime expectations.
...
* Keep encoding UTF-8; keep strict mode.
* Keep public symbol names unchanged.
...
6. Update `tsconfig.json`:
   - Prefer `extends: ../../tsconfig.json` (like other packages), OR set `compilerOptions.module` to `commonjs` explicitly.
   - Keep `outDir`, `rootDir`, `declaration` settings.
