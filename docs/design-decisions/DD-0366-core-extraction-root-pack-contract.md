...
`@balance-control/core` depends on `@balance-control/game` (runtime `dependencies`, for `EnginePackRegistry`, `EffectResolver`, shared types — identical to how `expansion-01/02/03` depend on `@balance-control/game` today). `packages/game` gets **zero** dependency back on `@balance-control/core`, at any dependency level (not even `devDependencies`). This is resolved structurally, not via a special case:
